...
export class SourceMapWatcher {
  ...
  public watchJsOutput(jsOutputPath: string): void {
    ...
    generateSourceMapDebounced(jsOutputPath);
  ...
  public generateSourceMap(
    ...
    const rootNode = jsTree.rootNode.firstNamedChild?.firstNamedChild?.firstNamedChild
    ...
    this.program.getForest().treeMap.forEach((sourceFile) => {
            ...
            switch (decisionTree.id) {
              case "Leaf":
                if (decisionTree.target.id === "Inline") {
                  const elmBranch = decisionTree.target.expr.childForFieldName(
                    "expr",
                  );

                  const elmPattern = decisionTree.target.expr.childForFieldName(
                    "pattern",
                  );
              ...
              case "Chain":
                {
                  ...
                  const jsElseBranch = jsNode.childForFieldName("alternative");
      ...
      function mapLetInExpr(elmNode: SyntaxNode, jsNode: SyntaxNode): void {
        elmNode.children
          ...
          .forEach((valueDeclaration) => {
            const jsFuncNode = jsNode.children.find(
              (n) =>
                n.type === "variable_declaration" &&
                n.firstNamedChild?.childForFieldName("name")?.text ===
                    ...
                    if (binaryTree.right.type === "Operand") {
                      ...
                      if (rightOperand.operand.type === "function_call_expr") {
                        ...
                        if (jsNode.type === "call_expression") {
                          ...
                          const jsArgs = jsNode.childForFieldName("arguments")
                            ?.firstNamedChild;
      ...
      function mapAnonymousFunctionExpr(
        ...
        const jsExprBody = jsNode.childForFieldName("body")?.firstNamedChild;

        if (elmExprBody && jsExprBody) {
          addMapping(elmExprBody, jsExprBody);
          mapExpr(elmExprBody, jsExprBody);
        }
      }
